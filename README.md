...
![Exception dashboard produced by the skill](docs/dashboard.svg)
...
## Run it on your real close

The demo proves the checks; your month-end just swaps in real exports. The whole loop is
**export → run → review → sign off**, and none of it touches your ERP.

1. **Export three reports from your ERP** as CSV (or Excel saved as CSV):
   - a **trial balance by entity** with current, prior, and budget columns
     (Workday: *Trial Balance* report, run twice or with comparative columns; QBO/NetSuite/Xero have direct equivalents),
   - the period's **journal-entry register / GL detail** with date, user, amount, and vendor,
   - your **balance-sheet reconciliation summary** (GL vs subledger with reconciling items and aging).
2. **Rename headers to the schema below** — on a copy, never on the source file. Ten minutes the
   first time; afterwards keep a saved export layout or a one-line mapping script.
3. **Run the analyzer with your materiality**, not the demo defaults. A rough starting point:
   `--flux_amt` ≈ your flux-explanation threshold, `--recon_item` ≈ your rec-review threshold,
   `--round_dollar_min` ≈ the approval limit above which a round-dollar manual JE deserves a look.
4. **Work the workpaper, top down.** Summary tab for scope, then clear each red row: attach support,
   write the real explanation over the drafted commentary, chase the intercompany break before
   consolidation. The memo becomes the shell of your close-package narrative.
5. **Sign off.** Nothing here posts or files; the output is a preparer draft until a qualified
   accountant approves it. Keep the generated workpaper with your close binder — it's already
   formatted as review evidence.

Cadence tip: run it on day 1 against the preliminary TB to get the exception list early, then
re-run after adjustments to confirm the flags cleared.

> **Work-computer note:** nothing in this repo needs installing on a corporate machine. Run it
> through Claude in the browser (the skill executes in Anthropic's cloud) and just upload the three
> CSVs — or run the script anywhere Python exists. Check your employer's data policy before
> uploading company financials to any external tool.
